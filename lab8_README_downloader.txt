...
Что можно написать в отчёте:
«В лабораторной работе реализована совместная обработка данных группой исполнителей. Сервер формирует очередь задач на скачивание информации из сети и передаёт исполнителям либо список адресов, либо диапазон числовых идентификаторов страниц. Исполнители независимо получают задачи, скачивают данные по URL и сохраняют их локально. Результаты выполнения передаются обратно серверу, который формирует итоговую сводку. При тестировании работа была показана на одном компьютере в локальной сети, что соответствует требованиям методических указаний.»
